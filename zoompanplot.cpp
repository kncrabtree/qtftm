...
QPair<double, double> ZoomPanPlot::getAxisAutoScaleRange(QwtPlot::Axis axis) const
{
    int i = getAxisIndex(axis);
    return qMakePair(d_config.axisList.at(i).min,d_config.axisList.at(i).max);
}
    ...
    if(obj == this->canvas())
    {
        ...
        else if(ev->type() == QEvent::MouseButtonRelease)
        {
            ...
            if(me != nullptr)
            {
                ...
                {
                    emit plotRightClicked(me->globalPos());
                    ev->accept();
                    return true;
                }
            }
...
int ZoomPanPlot::getAxisIndex(QwtPlot::Axis a) const
{
    int i;
    switch (a) {
